...
//  Cocktail
//
//  Created by spark-04 on 2024/03/05.
//
...
class CocktailDetailViewController: UIViewController, UIScrollViewDelegate {
    var imageURLs: [URL] = []
    var selectedIndex = 0
    ...
    func updateContentInset() {
        let imageSize = cocktailImageView.frame.size
    ...
    func adjustImageViewSize() {
        guard let imageSize = cocktailImageView.image?.size else { return }
        ...
        if scaledHeight > scrollViewHeight {
            ...
            cocktailImageView.frame.size = CGSize(width: scrollViewHeight * aspectRatio, height: scrollViewHeight)
        } else {
            // 高さが画面に収まる場合は、幅を画面に合わせる
            cocktailImageView.frame.size = CGSize(width: scrollViewWidth, height: scaledHeight)
        }
    }
